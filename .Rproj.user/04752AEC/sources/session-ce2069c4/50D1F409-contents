...
training_data <- rio::import('05_prepare_training_data/output/som_training_data.rds')|> 
  dplyr::filter(date >= "2015-01-01"& date <= END_DATE_ANALYSIS & p_time_u5 > 0) |> 
  tidyr::drop_na() 
...
# Save it
glm_fit |>
  saveRDS(paste('08_define_final_model/output/', 'som_glm_model_u5', annot, '.rds', sep=""))
...
boostrapping_results |>
  saveRDS("08_define_final_model/output/som_boostraping_data_u5.rds")
...
final_res |> 
  saveRDS("08_define_final_model/output/som_final_toll_dr_u5.rds")
