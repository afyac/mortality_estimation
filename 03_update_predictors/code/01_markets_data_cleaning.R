...
for (i in 1:nrow(outliers)) {
  ...
  mdat[which(mdat$y == outliers[i, "y"] & ! between(mdat[, x],
...
ts[, "date"] <- ymd(paste(ts$y, ts$m, "15", sep = "-"))
...
# Reshape long
complete <- melt(complete, id.vars = c("market", "y"), measure.vars = cols_prices)
...
# Reshape long
complete <- melt(complete, id.vars = c("district", "y"), measure.vars = cols_tot)
