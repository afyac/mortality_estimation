...
list_pred_to_try <- c('acled_event_rate_scn', 'cdi_lag5_scn', 'rainfall_lag5_scn',
                      ...
                      'sam_admissions_rate_cat', 'cholera_cases_rate_cat',
...
metrics <- c()
list_preds <- c()
for(nb_pred in seq(from = 10, to = 4)){
  ...
  for(ind in 1:ncol(list_preds_possible)){
    ...
    fit_formula <- as.formula(paste(pred_value," ~",'district|',
                                    paste(select_preds, collapse = " + "), '+ offset(log(', p_time_var, '))'))
    glm_fit <- glmertree::glmertree(formula = fit_formula, data = training_data, weights = qualityScore, family = 'poisson')
...
writexl::write_xlsx(results_final, 
                    '05_build_models/output/som_final_output_glmtree.xlsx')
