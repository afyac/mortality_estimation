...
training_data <- rio::import('05_prepare_training_data/output/som_training_data.rds')|> 
  dplyr::filter(date >= "2015-01-01" & p_time_u5 > 0 ) |> drop_na() 

predicting_data <- rio::import('06_prepare_predicting_data/output/som_predicting_data.rds')|> 
  dplyr::filter(date >= "2015-01-01") |> drop_na()

## Calculate folds
training_data <- training_data |> mutate(fold_unit = paste(district, cut(time_unit, 
...
f_cv_plot <- function(cv_data, plot_type = "scatter", 
    ...
    suppressMessages({
      result <- if ("scatter" %in% plot_type) {
        ...
        cv_data$cv_by_region_year |> 
          ...
          scale_color_viridis_d(option = "A", name = "Year") +
          theme_bw() +
          labs(
        ...
        resul <- cv_data$cv_by_region_year |> 
          mutate(bias_abs = pred - obs) |>
          ggplot(aes(
            x = 0, xend = bias_abs,
            y = reorder(region, bias_abs),
            group = region
          )) +
          ggalt::geom_dumbbell(
      ...
      if (!is.null(result) && save_plot) {
        ...
        ggsave(file_name, plot = result, dpi = "print", units = "cm", height = 21, width = 24)
      }
...
fit_formula <- as.formula(paste(pred_value," ~",
                                paste(list_pred_to_try, collapse = " + "), '+','(1|district)', '+ offset(log(', p_time_var, '))'))
...
#     
#     fit_formula <- as.formula(paste(pred_value," ~",
#                                     paste(select_preds, collapse = " + "), '+','(1|district)', '+ offset(log(', p_time_var, '))'))
#     glm_fit <- glmmTMB::glmmTMB(formula = fit_formula, data = training_data, weights = qualityScore,
#                                 family = "poisson")
#     
#     res <- f_cv_francesco(data_f = training_data, which_fold_f = "fold3",
...
# 
# 
# write.csv(results_final, '07_model_exploration/ouput/som_final_output_glmm.csv', row.names = FALSE)

f_cv_plot(res, plot_type = "scatter", 
                      dir_output_f = '08_define_final_model/visualization/output/',
                      save_plot = TRUE)
