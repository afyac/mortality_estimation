...
f_cv_francesco <- function(data_f = obs_fit, which_fold_f = "fold3",
  ...
  if (any(class(fit_f) %in% c("glm", "glm.nb", "zeroinfl", "glmmTMB",
    ...
    for (i in 1:nrow(out1) ) {
      ...
      out1[i, "obs"] <- sum(df_hold[, c(resp_var)])
...
list_pred_to_try <- c('acled_event_rate_scn', 'cdi_lag5_scn', 'rainfall_lag5_scn',
                      'measles_cases_rate_cat', 'tot_goat_cereal_smooth_lag3_scn', 
                      'cholera_cases_rate_cat',
                      'malaria_cases_rate_cat','tot_wage_cereal_smooth_lag3_scn', 
...
for(nb_pred in seq(from = 9, to = 5)){
  ...
  for(ind in 1:ncol(list_preds_possible)){
    ...
    print(select_preds)
    pred_cat <- select_preds[grepl('cat', select_preds, fixed = TRUE)]
    pred_non_cat <- select_preds[!grepl('cat', select_preds, fixed = TRUE)]
    fit_formula <- as.formula(paste(pred_value," ~", 
                                    paste('s(', pred_non_cat, ')', collapse = " + "),
                                    '+', paste(pred_cat, collapse = " + "),
                                    "+ offset(log(", p_time_var, "))"))
    glm_fit <- mgcv::gam(formula = fit_formula, data = training_data, weights = qualityScore,
                   family = "poisson")
...
write.csv(results_final, '07_model_exploration/ouput/som_final_output_gam.csv', row.names = FALSE)
